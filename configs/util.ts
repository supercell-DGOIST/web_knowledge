...
const forMap = (
  list: any[],
  callback: (item: any, index: number, list: any[]) => string | boolean | undefined
): any => {
  let index = 0
  const leng: number = list.length
  while (index < leng) {
...
const isVmNetwork = (mac: string): boolean => {
  const isvm: boolean | undefined = forMap(vmNetwork, (vm: string) => {
...
export const getLocalHost = (): string => {
  ...
  const host: string | undefined = forMap(dicts, (dict: NetworkInterfaceInfo) => {
